defmodule LdHost.Dictionary do
  ...
  in-band. Candidates are `.fs` files; promotion is a later reuse mark.
  Load is load-all. Grant-scoped retrieval is not a load path and is
  not authorization.
  ...
  def tautology?(tokens) when is_list(tokens) do
    if tokens == [] or Enum.any?(tokens, &(&1.kind != :word)) do
      false
    else
