defmodule LdHost.RunTest do
  ...
  test "seeded INSTALL rejects WRITE-FILE zipper pre-I/O" do
    ws = workspace()
    dict = System.tmp_dir!() |> Path.join("lddictcover-#{System.os_time(:nanosecond)}-#{System.unique_integer([:positive])}")
    seed_install(dict)
  ...
  test "follow-up INSTALL after covering miss writes the file" do
    ws = workspace()
    dict = System.tmp_dir!() |> Path.join("lddictfollow-#{System.os_time(:nanosecond)}-#{System.unique_integer([:positive])}")
    seed_install(dict)
    ...
    result = Run.run("greet", workspace: ws, contract: contract(), planner_fn: planner, dictionary_dir: dict, max_episodes: 3)
  ...
  test "first-episode define-INSTALL-then-call still promotes" do
    ws = workspace()
    ...
    assert result.success
    assert "INSTALL" in result.promoted_words
